...
def freeze_params_or_not(model, keys,freeze=True):
    logger.info('%sfreeze parameters name in %s'%('' if freeze else 'un',keys.__str__()))
    for k,v in model.named_parameters():
        for cur_key in keys:
            if k.find(cur_key)!=-1:
                #print('change %s requires_grad to %s'%(k,not freeze))
...
class VAE_KLD(torch.nn.Module):
    def __init__(self):
        super(VAE_KLD, self).__init__()

    def forward(self,mu,logvar):
        if MODEL_CONFIG.fb>0:
            kl_loss =0.5 * (mu.pow(2) + logvar.exp() - logvar - 1)
            dim_target_kl = float(args.fb)/float(args.vae_size)
            kl_mask = (kl_loss > dim_target_kl).float()
            fake_loss_kl = kl_mask * kl_loss
            return torch.sum(fake_loss_kl)/len(mu)
        else:
            return -(0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()))/len(mu)

def write_vae_loss(reconstruct_loss,kld):
    pass

def get_loss_dict(qa_loss, lm_loss,idt_loss=torch.tensor(0.),reconstruct_loss=torch.tensor(0.),kld=torch.tensor(0.)):
    loss_dict={}
    loss_dict['qa_loss']=qa_loss
    loss_dict['lm_loss'] = lm_loss
    loss_dict['idt_loss'] = idt_loss
...
def save_temp(t,tail=''):
    if isinstance(t,(list,tuple)):
        x = np.asarray([i.detach().cpu().numpy() for i in t])
    elif isinstance(t,torch.Tensor):
        x = t.detach().cpu().numpy()
    else:
        x = np.empty()
    with open('debug_%s.txt'%tail, mode='w',encoding='utf-8') as w:
        if isinstance(t,(list,tuple)):
...
def get_syntax_losses(parallel_model, cqa, Y, gen_X, gen_Y,idt_X, idt_Y, task_ids, is_extra, loss_fct, warmup, current_epoch,freeze_gpt):
    if "lll" in args.seq_train_type:
        ...
        qa_logits,_,qa_hidden_states,qa_attentions,qa_vae_results = parallel_model(cqa)[0]
        lm_logits,_,lm_hidden_states,lm_attentions,lm_vae_results = parallel_model(gen_X,task_ids=task_ids)[0]
        idt_logits,_,idt_hidden_states,idt_attentions,idt_vae_results = parallel_model(idt_X)[0]
        ...
        idt_loss =loss_fct([torch.transpose(l, 1, 2) for l in [idt_logits]], idt_Y) #if args.id_task_gamma>0 else torch.tensor(0.)


        if args.vae_idx>-1 and parallel_model.use_vae:
            vae_reconstruct_loss_fct = DataParallelCriterion(torch.nn.MSELoss(), args.device_ids)
            vae_kl_loss_fct = DataParallelCriterion(VAE_KLD(), args.device_ids)
            

            qa_vae_reconstructe_loss = vae_reconstruct_loss_fct(qa_vae_results[0],qa_hidden_states[args.vae_idx + 1])
            lm_vae_reconstructe_loss = vae_reconstruct_loss_fct(lm_vae_results[0], lm_hidden_states[args.vae_idx + 1])
            idt_vae_reconstructe_loss = vae_reconstruct_loss_fct(idt_vae_results[0], idt_hidden_states[args.vae_idx + 1])

            qa_vae_kld = vae_kl_loss_fct(qa_vae_results[1],qa_vae_results[2])
            lm_vae_kld = vae_kl_loss_fct(lm_vae_results[1], lm_vae_results[2])
            idt_vae_kld = vae_kl_loss_fct(idt_vae_results[1], idt_vae_results[2])

            f1 = 1.0 * (current_epoch - warmup['beta']['start_epoch']) / (
            ...
            vae_reconstructe_loss = qa_vae_reconstructe_loss+args.lm_lambda*lm_vae_reconstructe_loss+args.id_task_gamma*idt_vae_reconstructe_loss
            vae_kld = beta*(qa_vae_kld+args.lm_lambda*lm_vae_kld+args.id_task_gamma*idt_vae_kld)
        else:
            vae_reconstructe_loss = torch.tensor(0.)
            vae_kld = torch.tensor(0.)



        return get_loss_dict(torch.mean(qa_loss), args.lm_lambda * torch.mean(lm_loss),args.id_task_gamma*torch.mean(idt_loss),vae_reconstructe_loss,vae_kld)
    else:
        ...
        if args.vae_idx>-1 and parallel_model.use_vae:
            ...
            f1 = f1 * (1.0 * warmup['beta']['factor'])
            beta = torch.cuda.FloatTensor([min(max(f1, 0), warmup['beta']['factor'])])
            vae_reconstructe_loss = qa_vae_reconstructe_loss
            vae_kld = beta*qa_vae_kld
        else:
            vae_reconstructe_loss = torch.tensor(0.)
            vae_kld = torch.tensor(0.)
        ...
        return get_loss_dict(torch.mean(qa_loss), torch.tensor(0.),torch.tensor(0.),vae_reconstructe_loss,vae_kld)

def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct):
    if "lll" in args.seq_train_type:
        qa_logits = parallel_model(cqa)
        lm_logits = parallel_model(gen_X)
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in [qa_logits[0][0]]], Y)
        lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in [lm_logits[0][0]]], gen_Y)
        return get_loss_dict(torch.mean(qa_loss), args.lm_lambda * torch.mean(lm_loss))
    else:
        qa_logits = parallel_model(cqa)
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in [qa_logits[0][0]]], Y)
        return get_loss_dict(torch.mean(qa_loss), torch.tensor(0.))
...
def varlen_collate_fn(data):
    ...
    idt_Xs = torch.tensor(pad_all_to_max_len([datum[7] for datum in data], SPECIAL_TOKEN_IDS["pad_token"])).split(batch_size)
    idt_Ys = torch.tensor(pad_all_to_max_len([datum[8] for datum in data], FILL_VAL)).split(batch_size)
    task_ids = torch.tensor([datum[9] for datum in data]).split(batch_size)
    is_extra = torch.tensor([datum[-1] for datum in data]).split(batch_size)
    return list(cqs), list(len_cqs), list(cqas), list(len_cqas), list(Ys), list(gen_Xs), list(gen_Ys),list(idt_Xs),list(idt_Ys),list(task_ids),list(is_extra)
...
def dynamic_collate_fn(data, batch_size):
    ...
    def local_collate():
        ...
        null_counter = 0
        _cqs, _len_cqs, _cqas, _len_cqas, _Ys, _gen_Xs, _gen_Ys, _idt_Xs, _idt_Ys, _task_ids, _is_extra = [], [], [], [], [], [], [], [], [],[],[]
        Y_max_len = max(len(data[j][4]) for j in range(st, ed))
        cq_max_len = max(len(data[j][0]) for j in range(st, ed))
        idt_X_max_len = max(len(data[j][7]) for j in range(st, ed))
        idt_Y_max_len = max(len(data[j][8]) for j in range(st, ed))
        for j in range(st, ed):
            ...
            _idt_Xs.append(pad_to_max_len(data[j][7], idt_X_max_len-len(data[j][7]), SPECIAL_TOKEN_IDS["pad_token"]))
            _idt_Ys.append(pad_to_max_len(data[j][8], idt_Y_max_len - len(data[j][8]), FILL_VAL))
            _task_ids.append(data[j][9])
        ...
        idt_Xs.append(torch.tensor(_idt_Xs))
        idt_Ys.append(torch.tensor(_idt_Ys))
        task_ids.append(torch.tensor(_task_ids))
        is_extra.append(torch.tensor(_is_extra))

    cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, idt_Xs, idt_Ys, task_ids, is_extra = [], [], [], [], [], [], [], [], [], [],[]
    ...
    return cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, idt_Xs, idt_Ys,task_ids,is_extra
...
class QADataset(Dataset):
    ...
    def concat_example(self, gen_token, c, sep_token, q, ans_token, a, eos_token):
        ...
        if len(example) + 1 > args.max_len:#changed by Han Wang 2022/03/08
    ...
    def parse_example(self, gen_token, context, question, answer, idx):
        ...
        return cq_example, len(cq_example), cqa_example, len(cqa_example), Y_example, gen_X_example, gen_Y_example, idt_X_example, idt_Y_example, gen_token, idx
...
class WrapModel(torch.nn.Module):
    ...
    def forward(self, input_ids,task_ids=None):
        outputs = self.model(input_ids,task_ids=task_ids)
...
def sample_sequence(model, need_process, qa_results, all_pasts, max_tot_lens,task_ids=None):
    while len(need_process) > 0:
        ...
        while not stop:
            ...
            task_ids_temp=[] if task_ids is not None else None
            while True:
                try:
                    cur_id = next(it)
                    ...
                    if task_ids is not None:
                        task_ids_temp.append(task_ids[cur_id])
            ...
            if task_ids is not None:
                task_ids_temp = torch.stack(task_ids_temp)
            if args.model_name == "gpt2":
                for layer_id in range(MODEL_CONFIG.n_layer):
                    past[layer_id] = torch.stack(past[layer_id], dim=1)
                all_outputs = model(input_ids=input_ids.cuda(), past=past,task_ids=task_ids_temp)
            else:
...
def create_extra_data(task, prev_task, model, train_extra_data):
    ...
    if os.path.exists(gen_path) and not args.retrain:
    ...
    qa_results = []
    task_ids=[] if args.condition else None
    ...
    for task_name in args.tasks[:task_cnt]:
        ...
        if args.condition:#added by Han Wang 2022/1/14
            task_ids.extend([SPECIAL_TOKEN_IDS[task_name] for _ in range(gen_size//task_cnt)])
    #print(gen_size)
    #print(len(qa_results))
    if task_ids is not None:
        task_ids = torch.tensor(task_ids).cuda()
    ...
    for i in range(gen_size):
        ...
        if len(need_process) > int(args.memory_sizes[0] * 0.12):
            sample_sequence(model, need_process, qa_results, all_pasts, max_tot_lens,task_ids=task_ids)
    sample_sequence(model, need_process, qa_results, all_pasts, max_tot_lens,task_ids=task_ids)

    model.train()

    qa_results = [res.tolist() for res in qa_results]
    #qa_results = filter(assert_ans,qa_results)
...
def assert_ans(data):
    return SPECIAL_TOKEN_IDS['ans_token'] in data
