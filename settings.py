...
def parse_args():
    ...
    parser.add_argument("--n_train_epochs", type=int, default=24)
    ...
    #new
    parser.add_argument("--use_id_task", type=str2bool,default=True)
    parser.add_argument("--id_task_gamma", type=float, default=0.5)
    ...
    parser.add_argument("--output_hidden_states", type=str2bool, default=True)
    parser.add_argument("--output_attentions", type=str2bool, default=True)
    parser.add_argument("--reparameterize_with_noise", type=str2bool, default=True)
    parser.add_argument("--vae_idx", type=int, default=1)
    parser.add_argument("--vae_size", type=int, default=100)
    parser.add_argument("--bn_ln_vae", type=str, default='ln',choices=['bn','ln',''])
    parser.add_argument("--fb", type=int, default=20)
    parser.add_argument('--condition',type=str2bool,default=False,help='True is RVAE-LAMOL paper setting for RCVAE')
    parser.add_argument("--generate_mode",type=int,default=6,help='mode 6 is RVAE-LAMOL paper setting')
    parser.add_argument("--vae_version", type=str, default='1',choices=['1','2'],help='1 is RVAE-LAMOL paper setting')
    parser.add_argument("--mid_split",type=int ,default=2)
    parser.add_argument("--vae_lr",type=float,default=1e-4)
    parser.add_argument("--train_gap",type=int,default=4,help='4 is RVAE-LAMOL paper setting')
    parser.add_argument("--test_final",type=str2bool,default=False) 
    ...
    assert args.mid_split > 1,"Integer mid_split must bigger than 1,but now mid_split=%s"%args.mid_split
    
    #added by Han Wang 2022/1/21
    if args.generate_mode in [6,7]:
        turns = args.n_train_epochs//args.train_gap
        if turns%2>0:
            new_train_epochs = (1+turns)*args.train_gap
            logger.info('Mode %s ,train_epoch=%s is changed to %s for gpt2 and vae can be trained equally.'%(args.generate_mode,args.n_train_epochs,new_train_epochs))
            args.n_train_epochs = int(new_train_epochs)
    
    
    args.model_dir_root = os.path.join(args.model_dir_root, args.model_name,
            args.seq_train_type, "{}_{}{}{}{}{}{}{}{}{}".format(
                "_".join(args.tasks),
                args.gen_lm_sample_percentage,
                '_idt%s'%args.id_task_gamma if args.use_id_task else '',
                '_gen' if not args.add_task_tokens else '',
                '_vae%s_idx%s_size%s'%('' if args.vae_version is '1' else args.vae_version,args.vae_idx,args.vae_size) if args.vae_idx>-1 else '',
                '_%s'%args.bn_ln_vae if len(args.bn_ln_vae)>0 else '',
                '_fb%s'%args.fb if args.fb > 0 else '',
                '_cd' if args.condition else '',
                '_mode%s'%args.generate_mode if args.generate_mode>-1 else '',
                '_gap%s'%args.train_gap if args.generate_mode in [6,7] else "") if "lll" in args.seq_train_type else "_".join(args.tasks))

    args.device_ids = GPUtil.getAvailable(maxLoad=0.05, maxMemory=0.05, limit=args.n_gpus) if args.specified_device<0 else [args.specified_device]
    print('devices=',args.device_ids)
    ...
    if not args.fp32:
        ...
        if args.vae_idx>-1:
            MEMORY_FACTOR = dict([k, v*0.9] for k, v in MEMORY_FACTOR.items())#new
        else:
    ...
    model_config.output_attentions = args.output_attentions
    model_config.vae_idx = args.vae_idx
    model_config.vae_size = args.vae_size
    model_config.reparameterize_with_noise = args.reparameterize_with_noise
    model_config.bn_ln_vae = args.bn_ln_vae
    model_config.fb = args.fb
    model_config.condition = args.condition
    model_config.use_vae=True
    model_config.vae_version=args.vae_version

    tokens_weight = torch.ones([len(tokenizer)], dtype=torch.float).cuda()
