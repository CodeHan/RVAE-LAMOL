...
def test_one_to_one(task_load, task_eval, model, score_dict):
    ...
    if args.seq_train_type=='multitask':
        test_qadata = QADataset(TASK_DICT[task_eval]["test"] , "test", SPECIAL_TOKEN_IDS[args.tasks[0]]).sort()
    else:
    ...
    cnt = 0
    for n_steps, (cqs, len_cqs, _, _, _, _, _, _, _, task_ids, _) in enumerate(test_dataloader):
        # assume n_gpus == 1
        cqs = cqs[0]
        len_cqs = len_cqs[0]
        n_inputs = cqs.shape[0]
        all_outputs = model(input_ids=cqs.cuda())#,task_ids=task_ids.cuda())
    ...
    for i in range(len(test_qadata)):
        _, len_cq, _, _, Y, _, _, _,_,_,_,_ = test_qadata[i]
...
def test_one_to_many(task_load):
    score_dicts = []
    eps = [args.n_train_epochs[task_load]-1] if args.test_final else [i for i in range(args.n_train_epochs[task_load])]
    for ep in eps:#range(args.n_train_epochs[task_load]):
        ...
        if args.seq_train_type=='multitask':
            gen_token = get_gen_token(args.tasks[0])
            TOKENIZER.add_tokens([gen_token])
            SPECIAL_TOKENS[args.tasks[0]] = gen_token
            SPECIAL_TOKEN_IDS[args.tasks[0]] = TOKENIZER.convert_tokens_to_ids(gen_token)
        else:
